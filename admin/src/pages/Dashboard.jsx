...
import { TrendingUp, IndianRupee, Users, Package, Edit, Trash2, Layers } from 'lucide-react';
...
export const Dashboard = () => {
  ...
  const totalProducts = analytics ? analytics.totalProducts : products.length;
  const totalCustomers = analytics ? analytics.totalCustomers : 0;
  const totalRevenue = analytics ? analytics.totalRevenue : 0;
  const totalInventoryValue = analytics ? analytics.totalInventoryValue : 0;
  const chartData = analytics && analytics.chartData ? analytics.chartData : [];

  const stats = [
    {
      label: 'Catalog Revenue Value',
    ...
    },
    {
      ...
      value: loading ? '...' : `₹${totalInventoryValue.toLocaleString('en-IN')}`,
      change: 'Live DB',
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-8">
      <div>
        ...
        <p className="text-zinc-400 mt-1">Real-time storefront metrics and product catalog data.</p>
      </div>
        ...
        {stats.map((stat, idx) => (
          <motion.div key={idx} whileHover={{ y: -5 }} className="glass-panel p-6 relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start mb-4">
              ...
              <span className={`text-xs font-semibold px-2 py-1 rounded-full ${stat.positive ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400'}`}>
                {stat.change}
              </span>
            </div>
            <div>
        ...
        <div className="lg:col-span-2 glass-panel p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-0.5">Real monthly catalog revenue pipeline from database</p>
            </div>
            <span className="text-xs font-mono text-primary-400 bg-primary-500/10 border border-primary-500/20 px-3 py-1 rounded-full">
              Live Backend Sync
            </span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-4 max-h-[320px]">
            {loading ? (
              ...
              <div className="h-full flex items-center justify-center text-center">
                <p className="text-zinc-500 text-sm">No products found in database.</p>
              </div>
            ) : (
                ...
                <div key={product._id} className="flex items-center justify-between p-3 rounded-xl hover:bg-surfaceHover border border-transparent hover:border-border transition-all group relative">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-400">₹{Number(product.price).toLocaleString('en-IN')}</p>
                    </div>
                  </div>
