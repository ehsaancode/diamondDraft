  ...
  return (
    ...
    >
      <nav className="flex items-center justify-between px-4 md:px-8 py-3.5 max-w-7xl mx-auto w-full">
        ...
        {/* Right Actions Container (Mobile & Desktop) */}
        <div className="flex items-center gap-1.5 md:gap-3">
          ...
          <motion.button
            ...
            className="p-2 text-gray-800 hover:bg-gray-100 rounded-full transition-colors cursor-pointer"
          >
            <Search size={20} className="text-gray-800" strokeWidth={1.5} />
          </motion.button>

          {/* Wishlist Heart Icon (Mobile & Desktop) */}
          <motion.button
          ...
          >
            ...
            {favoriteCount > 0 && (
              <motion.span
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className="absolute top-0 right-0 bg-red-500 text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center shadow-xs"
              >
                {favoriteCount}
              </motion.span>
            )}
          ...
          {/* Cart Animated Icon (Mobile & Desktop) */}
          <motion.button
            whileHover={{ scale: 1.15, rotate: -5 }}
            whileTap={{ scale: 0.88 }}
          ...
          >
            ...
            {cartCount > 0 && (
              <motion.span
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className="absolute top-0 right-0 bg-black text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center shadow-xs"
              >
                {cartCount}
              </motion.span>
            )}
